...
#'   (\file{solita.css} and \file{solita-fonts.css}) are provided in this
...
#'   e.g., for \code{css = c('solita', 'extra.css')}), it means
#'   \code{solita.css} in this package and a user-provided \code{extra.css}. To
#'   find out all built-in CSS files, use \code{solitaslides:::list_css()}. With
...
#'
#'   When the slides are previewed via \code{solitaslides::\link{inf_mr}()},
...
#' @export
#' @examples
#' # rmarkdown::render('foo.Rmd', 'solitaslides::moon_reader')
moon_reader = function(
  css = c('solita', 'solita-fonts'), self_contained = FALSE,yolo=FALSE,
  ...
  tmp_js = tempfile('solitaslides', fileext = '.js')  # write JS config to this file
  tmp_md = tempfile('solitaslides', fileext = '.md')  # store md content here (bypass Pandoc)
  if (self_contained && isTRUE(getOption('solitaslides.inf_mr.running'))) {
    if (interactive()) xfun::do_once({
      message(
        'You are currently using solitaslides::inf_mr() to preview your slides, and ',
        'you have turned on the self_contained option in solitaslides::moon_reader. ',
    ...
    }, 'solitaslides.self_contained.message')
  ...
  xfun::write_utf8(as.character(tagList(
    ...
    tags$script(HTML(paste(c(sprintf(
      ...
      'show-widgets', 'print-css', 'after', 'script-tags', 'target-blank', 'snow'
    ))),
...
infinite_moon_reader = function(moon, cast_from = '.', ...) {
  ...
  build = local({
    ...
    files = if (getOption('solitaslides.inf_mr.aggressive', TRUE)) function() {
      c(list.files(
        d, '[.](css|js|png|gif|jpeg)$', full.names = TRUE, recursive = TRUE
      ), moon)
    } else function() moon
    mtime = function() file.info(files())[, 'mtime']
    html <<- xfun:::normalize_path(rebuild())  # render Rmd initially
    ...
    function(message) {
      ...
      if (u) {
        ...
        if (identical(xfun:::normalize_path(as.character(ctx[['path']])), moon)) {
  ...
  d = xfun:::normalize_path(cast_from)
...
#' (including slides produced by \pkg{solitaslides}).
...
#'   solitaslides::decktape('https://slides.yihui.org/xaringan', 'xaringan.pdf', docker = FALSE)
#' }
decktape = function(
